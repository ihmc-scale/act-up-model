...
COPY evacuation-model-v1.0.2.lisp ./evacuation-model-v1.0.2.lisp
COPY model-server.lisp ./model-server.lisp
COPY docker-entrypoint.sh ./docker-entrypoint.sh
RUN chmod a+x docker-entrypoint.sh

# get the dependencies compiled
RUN sbcl --load quicklisp/setup.lisp --load act-up-v1_3_2 --load 'evacuation-model-v1.0.2.lisp' --load model-server.lisp --eval '(sb-ext:exit)'
